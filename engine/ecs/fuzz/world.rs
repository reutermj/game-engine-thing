//! libFuzzer target: structural changes and layout migrations on a world,
//! against the model (`ecs_ops::world`), with the input as the driver's
//! choices. A failed check panics, which aborts, which libFuzzer reports
//! with the input that did it.
