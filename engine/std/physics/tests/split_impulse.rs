...
//! Box2D has them. The split impulse is Bullet's (Erwin Coumans's push
//! velocities). See docs/CREDITS.md.
...
impl SolverBody {
    ...
    pub fn new(v: Vec2, inv_mass: f32, _gravity: Vec2) -> SolverBody {
        SolverBody { v, inv_mass, pseudo: Vec2::ZERO }
    }

    /// How far a dynamic body moves this step.
    pub fn displacement(&self, dt: f32) -> Vec2 {
        (self.v + self.pseudo) * dt
    }
}
