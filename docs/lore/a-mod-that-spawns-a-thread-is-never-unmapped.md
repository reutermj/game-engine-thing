...
still pins the library is, inferred rather than traced: `std::thread::spawn`
records the *spawning* thread's handle in a thread-local of the mod's own copy
of std, which registers a TLS destructor on the spawning thread, here the
engine's main thread. glibc won't unmap a library with TLS destructors still
registered, and a main-thread destructor only runs at exit.
